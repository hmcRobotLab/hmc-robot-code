...
class Ardrone():
  def __init__(self,controlName="ardrone2/heli", navPubName="ardrone2/navData"):
    ...
    3: save sliders           =: movement based on save sliders
    """
    ...
    self.slider_yaw = 50
    self.slider_angle = 0
    self.slider_keyPower = 7

    ## Initial vaalues
    self.phi_for_drone = 0
    self.theta_for_drone = 0
    self.gaz_for_drone = 0
    self.yaw_for_drone = 0
  ...
  def make_slider_window(self):
    ...
    cv.CreateTrackbar('gaz', 'sliders', self.slider_gaz, 100, self.change_gaz)
    cv.CreateTrackbar('yaw', 'sliders', self.slider_yaw, 100, self.change_yaw)
    cv.CreateTrackbar('angle', 'sliders', self.slider_angle, 360, self.change_angle)
    cv.CreateTrackbar('keyPower', 'sliders', self.slider_keyPower, 15, self.change_keyPower)
      

  def change_gaz(self, new_threshold):
    self.slider_gaz = new_threshold
  def change_yaw(self, new_threshold):
    self.slider_yaw = new_threshold
  def change_angle(self, new_threshold):
    self.slider_angle = new_threshold
  def change_angle(self, new_threshold):
    self.slider_angle = new_threshold
  def change_keyPower(self, new_threshold):
    self.slider_keyPower = new_threshold

  def save_sliders_to_robot(self):
    self.keyPower = float(self.slider_keyPower)/100

    self.phi_for_drone = (math.cos(math.radians(self.slider_angle)))*self.keyPower
    
    self.theta_for_drone = (math.sin(math.radians(self.slider_angle)))*self.keyPower
    
    self.gaz_for_drone = (float(self.slider_gaz-50)/50)*self.keyPower
    
    self.yaw_for_drone = (float(self.slider_yaw-50)/50)*self.keyPower

    if -1.00e-16<self.theta_for_drone < 1.00e-16:
      self.theta_for_drone = 0
    if -1.00e-16<self.phi_for_drone < 1.00e-16:
      self.phi_for_drone = 0
      
    print self.keyPower
    print self.phi_for_drone
    print self.theta_for_drone
    print self.gaz_for_drone
    print self.yaw_for_drone
    print '_______'
  ...
  def keyCmd(self,char):
    ...
    if char == ' ':
      ...
      if char == 'w':
        ...
        sflag = 1
        stheta = self.theta_for_drone
        sphi = self.phi_for_drone
      elif char == 'z':
        sflag = 1
        stheta = self.theta_for_drone
        sphi = self.phi_for_drone 
        ...
        syaw = self.yaw_for_drone
      elif char == 'f':
        syaw = self.yaw_for_drone
      elif char == 'v':
        sgaz = self.gaz_for_drone
      elif char == 'b':
        sgaz = self.gaz_for_drone
      elif char == '=':
        sflag = 1
