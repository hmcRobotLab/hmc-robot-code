...
class Mcl:
  ...
  def randParticle(self):
    return Particle(random()*self.maxX,random()*self.maxY,random()*2*pi,transNoise=0.1,rotNoise=0.1,laserNoise = self.mapRes * 2)
  ...
  def resetDraw(self):
    ...
    #cv2.cvtColor(self.drawMap,self.gMap,cv2.COLOR_GRAY2RGB)
...
def test():
  ...
  while True:
    ...
    while not goodP: 
      ...
      if min(mcl.getScan(testParticle)) > 2.0:
        goodP = True
