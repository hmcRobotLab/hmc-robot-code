...
def process_section():
    ...
    if len(D["sections"]) > 0:
        ...
        adds.extend([[x[1], x[2]] for x in D["sections"] if x[0] == 0])

        # pull out all sections with a 's' in the front
        subs = [[x[1], x[2]] for x in D["sections"] if x[0] == 's']

        # pull out all sections with integers in the front
        mul = [[x[0], x[1], x[2]] for x in D["sections"] if isinstance(x[0], int)]
        D['multhresh'] = [{'low_red':0, 'high_red':255,\
                       'low_green':0, 'high_green':255,\
                       'low_blue':0, 'high_blue':255,\
                       'low_hue':0, 'high_hue':255,\
                       'low_sat':0, 'high_sat':255,\
                       'low_val':0, 'high_val':255 } for x in xrange(len(mul))]
        # print "D['multhresh'] = ", D['multhresh']

        print "adds: ", adds
        print "subs: ", subs
        print "multiple adds: ", mul
        # mins default to highest value, maxs default to lowest value
        minmax = [[255,0] for x in xrange(6)]
        minmaxmul = [[[255,0],[255,0],[255,0],[255,0],[255,0],[255,0]] \
                     for x in xrange(len(mul))]
        # print minmax
        # print minmaxmul
            ...
            for x in xrange(x0, x1):
                for y in xrange(y0, y1):
                    ...
                    if in_range:
                        ...
                        #print allv
                        for j in xrange(len(allv)):
                            if allv[j] < minmaxmul[i][j][0]:
                                minmaxmul[i][j][0] = int(allv[j])
                            if allv[j] > minmaxmul[i][j][1]:
                                minmaxmul[i][j][1] = int(allv[j])
                        # print minmaxmul[i]

            # print i,  D["multhresh"][i]
            # print "minmaxmul part i= ", minmaxmul[i]
            # print minmaxmul
            for k in xrange(len(names)):
                for j in xrange(2):
                    D["multhresh"][i][names[k][j]] = minmaxmul[i][k][j]
            # print D["multhresh"]

def threshold_mulimage(i):
    """ runs the image processing in order to create a 
        black and white thresholded image out of D["image"]
        into D["threshed_image"]
    """
    ...
    # Here is how OpenCV thresholds the images based on the slider values:
    cv.InRangeS(D["red"], D['multhresh'][i]["low_red"], \
                    D['multhresh'][i]["high_red"], D["red_threshed"])
    cv.InRangeS(D["blue"], D['multhresh'][i]["low_blue"], \
                    D['multhresh'][i]["high_blue"], D["blue_threshed"])
    cv.InRangeS(D["green"], D['multhresh'][i]["low_green"], \
                    D['multhresh'][i]["high_green"], D["green_threshed"])
    cv.InRangeS(D["hue"], D['multhresh'][i]["low_hue"], \
                    D['multhresh'][i]["high_hue"], D["hue_threshed"])
    cv.InRangeS(D["sat"], D['multhresh'][i]["low_sat"], \
                    D['multhresh'][i]["high_sat"], D["sat_threshed"])
    cv.InRangeS(D["val"], D['multhresh'][i]["low_val"], \
                    D['multhresh'][i]["high_val"], D["val_threshed"])
...
def onMouse(event, x, y, flags, param):
    ...
    if event==cv.CV_EVENT_LBUTTONDOWN: # clicked the left button
        ...
        if D["mode"] == "start":
            D["sections"] = [ [0, D["up_coord"], D["down_coord"]] ]
            process_section()
        elif D["mode"] == "add":
            ...
            process_section()
        elif D["mode"] == "multiple" and len(D['sections'])<4:
            D["sections"].append([len(D["sections"]), D["up_coord"], D["down_coord"]])
            process_section()
        elif D["mode"] == "multiple" and len(D['sections'])>=4:
            print "max. number of discernable colors reached."
        elif D["mode"] == "clear":
            D["sections"] = []
    ...
    if key_press == ord('q') or key_press == 27: # if a 'q' or ESC was pressed
        ...
        if len(D["sections"]) > 0:
            ...
            if len(D["sections"])>1:
                ...
                D['vectors']['showobs'] = True
                D['target']['targeting'] = True
                D['obstacle']['xyobs'] = D['mulcenter'][1]
                print D['obstacle']['xyobs']
        else:
            print "Cannot switch modes, need a starting area first. Press 'i' " + \
                "to select a starting area."
        ...
        if len(D["sections"])>0:
            find_center(0)
            D["multiple_number"] = 1
            print"center of zone 1 is", D["mulcenter"][0]
        else:
            print "Cannot display center for area 1, need a starting area first. Press 'i' " + \
                "to select a starting area."

    elif key_press == ord('2'):
        if len(D["multhresh"])>1:
            find_center(1)
            D["multiple_number"] = 2
            print"center of zone 2 is",  D["mulcenter"][1]
        else:
            print "Cannot display center for area 2, need to select area 2. Press 'l' " + \
                "to select the next area."
            
    elif key_press == ord('3'):
        if len(D["multhresh"])>2:
            find_center(2)
            D["multiple_number"] = 3
            print"center of zone 3 is",  D["mulcenter"][2]
        else:
            print "Cannot display center for area 3, need to select area 3. Press 'l' " + \
                "to select the next area."
            
    elif key_press == ord('4'):
        if len(D["multhresh"])>3:
            find_center(3)
            D["multiple_number"] = 4
            print"center of zone 4 is",  D["mulcenter"][3]
        else:
            print "Cannot display center for area 4, need to select area 4. Press 'l' " + \
                "to select the next area."
        ...
        if len(D["multhresh"])>1:
            ...
            print "Cannot display centers for area 1 and 2, need to select area 2. Press 'l' " + \
                "to select the next area."
...
def FSM1(timer_event=None ):
    ...
    if D['state'] == "Keyboard": # user stopped our state machine!
        ...
        distancegiver() # not really needed, but we kept it here. 
        return
        ...
        anglerot = anglerob - angletar #the angle to rotate
        
        
        if anglerot < 0:
            angleone = 360-abs(anglerot) #finding the aternate angle
        else:
            angleone = anglerot-360 #finding the alternate angle


        if(abs(angleone)<abs(anglerot)): #see which has smaller angle of magnitude
            anglerot = angleone
            
        timeforrot = math.fabs(anglerot)/30
        # approximate the angle -- usually we use 22
        # instead of 30 but our robot did better with 30.
