...
class DroneController:
    ...
    def __init__(self, use_drone = False):
        ...
        self.thresholds = {'low_red': 130, 'high_red': 256,\
                           'low_green': 0, 'high_green': 64, \
                           'low_blue': 0, 'high_blue':84, \
    ...
    def init_drone_parameters(self):
        ...
        if self.use_drone == True:
            ...
            self.drone.cam = 0 # 0 for forward, 1 for floor camera
    ...
    def find_biggest_region(self):
        ...
        # Next we want to find the *largest* contour
            ...
            self.center = [br[0] + br[2]/2, br[1] + br[3]/2]
            self.length = br[3]
            ...
            cv.PolyLine(self.color_image,[[(br[0],br[1]),(br[0] + br[2],br[1]),
                                     (br[2]+br[0],br[1]+br[3]),
                                     (br[0], br[1] + br[3])]],
                                        1, cv.RGB(255, 0, 0))
                        
            #Example of drawing a yellow circle
            cv.Circle(self.color_image,(br[0] + br[2]/2, br[1] + br[3]/2), 10, cv.RGB(0, 0, 255),\
                      thickness=1, lineType=8, shift=0)
            
            
            #Draw the contours in white with inner ones in green
            cv.DrawContours(self.color_image, biggest, cv.RGB(0,255,0),
                            cv.RGB(0, 0, 255), 1, thickness=2, lineType=8,
                            offset=(0, 0))  
