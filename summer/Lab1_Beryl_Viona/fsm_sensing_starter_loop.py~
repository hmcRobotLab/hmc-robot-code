...
def state_forward( timer_event = None ):
    ...
    tank( 0, 100, 100)
    transition( 0.1, state_bump_wait )

def state_bump_wait( timer_event = None ):
    """ waits for a bump on the right bumper """
    print "Now waiting for right bump"
    if data.bumpRight:
        tank(0,-100, 100)
        transition( 1.0, state_forward)
    else:
        transition( 0.1, state_bump_wait)
